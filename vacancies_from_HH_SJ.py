...
class HeadHunterAPI(AbstractClass):
    def __init__(self, keyword):
        self.__header = {"User-Agent": "My_App"}
        self.__params = {
            "text": keyword,
            "page": 0,
            "per_page": 100
    ...
    def get_vacancies(self, pages_count=1):
        while self.__params['page'] < pages_count:
            print(f"Парсинг HeadHunter, {self.__params['page'] + 1} стр.", end=": ")
            try:
                values = self.get_request()
            except ParsingError:
                print('Ошибка получения данных')
                break
            print(f"Получено вакансий: {len(values)}.")
...
class SuperJobAPI(AbstractClass):
    ...
    def get_vacancies(self, pages_count=1):
        while self.__params['page'] < pages_count:
            print(f"Парсинг SuperJob, {self.__params['page'] + 1} стр.", end=": ")
            try:
                values = self.get_request()
            except ParsingError:
                print('Ошибка получения данных')
